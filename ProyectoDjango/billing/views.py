...
from shared.decorators import audit_action, roles_required, user_has_role
from shared.emails import send_welcome_email
from shared.microservicios import notificar_facturacion
...
def invoice_confirm(request, pk):
    ...
    if request.method == 'POST':
        ...
        # Solo Vendedor/Administrador disparan el comprobante SRI del
        # microservicio de facturación — ver shared/decorators.py::user_has_role.
        if user_has_role(request.user, 'Administrador', 'Vendedor'):
            metodo_pago = 'credito' if tipo_pago == Invoice.CREDITO else 'efectivo'
            notificar_facturacion(invoice=invoice, metodo_pago=metodo_pago)
...
class BrandListView(LoginRequiredMixin, GroupRequiredMixin, SearchExportMixin, ListView):
    ...
    export_pdf_perm   = 'billing.export_brand_pdf'
    export_excel_perm = 'billing.export_brand_excel'
    export_fields   = [
...
class ProductGroupListView(LoginRequiredMixin, GroupRequiredMixin, SearchExportMixin, ListView):
    ...
    export_pdf_perm   = 'billing.export_productgroup_pdf'
    export_excel_perm = 'billing.export_productgroup_excel'
...
class SupplierListView(LoginRequiredMixin, GroupRequiredMixin, SearchExportMixin, ListView):
    ...
    export_pdf_perm   = 'billing.export_supplier_pdf'
    export_excel_perm = 'billing.export_supplier_excel'
    export_fields   = [
...
class CustomerListView(LoginRequiredMixin, SearchExportMixin, ListView):
    ...
    export_pdf_perm   = 'billing.export_customer_pdf'
    export_excel_perm = 'billing.export_customer_excel'
    export_fields   = [
