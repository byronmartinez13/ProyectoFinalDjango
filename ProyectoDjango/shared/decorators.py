...
def user_has_role(user, *group_names):
    """
    Chequeo puntual de rol (NO es un decorador de vista): True si el usuario
    es superusuario o pertenece a alguno de los grupos indicados.

    A diferencia de roles_required(), esto no bloquea el acceso a una vista
    completa — sirve para condicionar un bloque de código dentro de una
    vista ya accesible (p. ej. si se dispara o no una llamada a un
    microservicio externo según el rol de quien hizo la acción).
    """
