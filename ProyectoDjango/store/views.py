...
from shared.microservicios import notificar_facturacion, notificar_pago
...
def paypal_capture_order(request, paypal_order_id):
    ...
    # Delegar a los microservicios independientes de pagos/facturación.
    # Puramente informativos — el pago real ya fue capturado en PayPal y la
    # factura ya quedó emitida en la BD local arriba, así que si alguno está
    # caído la venta NO se revierte (ver shared/microservicios.py).
    notificar_pago(
        correo=invoice.customer.email,
        monto=invoice.total,
        metodo=payment_method,
        nombre_cliente=invoice.customer.full_name,
    )
    notificar_facturacion(invoice=invoice, metodo_pago='tarjeta')
