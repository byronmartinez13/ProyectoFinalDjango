...
from shared.decorators import roles_required, user_has_role
from shared.microservicios import notificar_abono
...
def cobro_create(request, factura_pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Solo Vendedor/Administrador disparan el comprobante del
            # microservicio de pagos — ver shared/decorators.py::user_has_role.
            if user_has_role(request.user, 'Administrador', 'Vendedor'):
                notificar_abono(
                    correo=cobro.factura.customer.email,
                    factura_id=cobro.factura_id,
                    monto_abono=cobro.valor,
                    saldo_restante=cobro.factura.saldo,
                    nombre_cliente=cobro.factura.customer.full_name,
                )
