...
- **Facturación electrónica** — al emitir cualquier factura (venta manual o checkout de la Tienda), se envía automáticamente por correo al cliente con el PDF de la factura adjunto.
...
- Enviar correo transaccional (bienvenida, recuperación de contraseña y facturación electrónica) vía SMTP
...
│   ├── services.py                  ← check_stock(), emit_invoice() (dispara la factura
│   │                                   electrónica vía shared.emails), recalc_invoice()
│   │                                   (reutilizado por Ventas y por el checkout de la Tienda)
│   ├── pdf.py                       ← build_invoice_pdf(): genera el PDF de una factura
│   │                                   (ReportLab), reutilizado por la descarga manual y
│   │                                   por el correo de facturación electrónica
│   ├── forms.py                     ← SignUpForm (Admin, con selector de rol), BrandForm,
│   │                                   InvoiceForm, InvoiceDetailFormSet, CreditNoteForm
│   │                                   (valida monto contra saldo disponible de la factura)
...
│   ├── tests.py                     ← Tests de validadores, envío de factura electrónica
│   │                                   y validación de montos de Notas de Crédito
│   ├── migrations/
│   └── templates/billing/
...
| `Invoice`       | FK → Customer. `estado`: Borrador(0) / Emitida(1) / Anulada(2). **`payment_method`** ('card'/'paypal') y **`paypal_order_id`**: solo se llenan cuando la factura nace de un checkout online. Al emitirse (`estado=EMITIDA`) dispara el envío automático de la factura electrónica al correo del cliente |
...
- **Emitida** — el stock se descuenta automáticamente al emitir; solo se puede anular, crear nota de crédito o sustituir. Se envía automáticamente la **factura electrónica** por correo al cliente (PDF adjunto).
...
- La emisión (descuento de stock + `StockMovement` + envío de factura electrónica) vive en `billing/services.py::emit_invoice()`, compartida entre la emisión manual (Vendedor/Administrador) y el checkout online de la Tienda: **un único punto** que garantiza que ambos flujos se comporten igual.

#### Facturación electrónica

Al emitirse una factura (`emit_invoice()`), `shared/emails.py::send_invoice_email()` genera el PDF con `billing/pdf.py::build_invoice_pdf()` (el mismo generador que usa la descarga manual) y lo envía por correo al cliente:

- Se dispara automáticamente en **ambos** flujos de emisión: venta manual (Vendedor/Administrador) y checkout de la Tienda (PayPal/Tarjeta).
- Si el cliente no tiene correo registrado, se omite el envío y se registra un aviso en el log — la emisión de la factura **nunca** se bloquea por esto.
- Si el envío falla (SMTP caído, credenciales inválidas, etc.), el error se registra en el log y se atrapa en silencio, igual que `send_welcome_email()`: nunca revierte la emisión de la factura ni el descuento de stock.
- Con `EMAIL_HOST` vacío en `.env` (backend de consola), el correo con el PDF adjunto se imprime en la terminal — útil para probar el flujo completo sin credenciales SMTP reales.
...
| Confirmación      | Al capturar el pago se genera la factura, se descuenta el stock, se vacía el carrito y se envía la factura electrónica al correo del cliente |
...
| Correo transaccional       | Bienvenida (usuario/contraseña/rol), recuperación de contraseña y **facturación electrónica** (PDF de la factura adjunto al emitir), vía Gmail SMTP |
...
| Validaciones de negocio    | Nombres solo letras (`validate_only_letters`), teléfono ecuatoriano (`validate_phone_ec`), precios/montos > 0, cantidades ≥ 1, motivos con longitud mínima, fechas de pago acotadas al rango válido (ni futuras ni previas al documento), y monto de notas de crédito limitado al saldo disponible de la factura/compra |
...
### `send_invoice_email()`
Envía la factura electrónica (PDF adjunto, generado con `billing/pdf.py::build_invoice_pdf()`) al correo del cliente. Se llama automáticamente desde `billing/services.py::emit_invoice()` al emitir cualquier factura. Si el cliente no tiene correo o el envío falla, registra el aviso en el log y no interrumpe la emisión (mismo criterio de "fallo silencioso" que `send_welcome_email()`).

### `validate_cedula_ec`
Valida que el campo `dni` sea matemáticamente correcto según el algoritmo del Registro Civil del Ecuador.

### `validate_only_letters` / `validate_phone_ec`
`validate_only_letters` exige que el campo contenga solo letras (con tildes y Ñ, espacios o guiones entre palabras) — usado en nombres/apellidos de `Customer`, `Supplier.contact_name` y en los formularios de registro de usuario. `validate_phone_ec` exige un teléfono ecuatoriano válido (convencional de 7-9 dígitos, celular de 10 dígitos, o formato `+593` + 9 dígitos) — usado en `Customer.phone` y `Supplier.phone`.
...
**Prompt 6**
```
Añade la función de facturación electrónica: que una vez realizada la
compra en el módulo Cliente, o creada la factura, se envíe al correo del
cliente automáticamente. Luego revisa las validaciones (fechas, números,
letras, campos obligatorios, campos vacíos, cualquier validación que
pueda revisar un profesor en un proyecto final de semestre), sin alterar
los comportamientos ya establecidos por los demás procesos.
```
